...
### Example Command: Rscript wilcoxon-test.R matfile.mat cell,cell,cell,exosomes,exosomes,exosomes /projects/project_name mature_sense_minExpr5_RCadj
...
sink(paste(outdir, "/wilcoxontest_", basename, ".log", sep=""), append = TRUE)

print("Wilcoxon-test is now running...")

# Performing CPM transformation on the raw data
transf_table <- cpm(matfile, log=F, prior.count=0)

# Applying the paired exact Wilcoxon-test 
stat_value <- as.data.frame(apply(transf_table, 1, function(x)
              wilcox.exact(x[which(groups==sampletypevalues[1])], x[which(groups==sampletypevalues[2])], paired=T, exact=T)$p.value))
...
print(paste("Exporting wilcoxon-test mean normalised results per group per (all) gene to: ", outdir,"/",basename,"_",sampletypevalues[1],"VS",sampletypevalues[2],"_wilcoxontest_meanGroupsAllGenes.csv", sep=""))
write.table(data.frame("name"=rownames(mean_ncounts_selected), mean_ncounts_selected), file=paste(outdir,"/",basename,"_",sampletypevalues[1],"VS",sampletypevalues[2],"_wilcoxontest_meanGroupsAllGenes.csv", sep=""), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)


## Wilcoxon-testing analysis finished
print("Wilcoxon-testing analysis has finished successfully!")

### Wilcoxon-testing analysis is now being terminated...
print(" ---------------------------------------------- Wilcoxon-testing ANALYSIS FINISHED ---------------------------------------------- ")
